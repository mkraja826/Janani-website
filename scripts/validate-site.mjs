...
const giving = fs.readFileSync(path.join(root, 'giving/index.html'), 'utf8');
for (const phrase of ['No verified donations', 'public_giving_ledger', 'A Care+ purchase is not a charitable donation']) {
  if (!giving.includes(phrase)) throw new Error(`Giving page missing required transparency language: ${phrase}`);
}
